...
void addSymbol (bool isIdent, const string &lexeme, const string &type,const string &value) {
    ...
    symbolTable[symbolCount].lvalue = rand(); //initialize address
    symbolCount++;
}
...
void generateIL(Node* node){
    ...
    if(node-> token == T_ASSIGN){
        ...
        //convert types if actual type of operands is different from result type of node (expected type)
        if(node -> right -> actual_type == "int" && node->left->expected_type == "float") {
            cout << "itof" << "\n";
        } else if(node -> right -> actual_type == "float" && node->left->expected_type == "int") {
            cout << "ftoi" << "\n";
        } //because my assign list is bugged this won't work
    ...
    if (node->token >= T_ADD && node->token <= T_DIV) {
        ...
        //save values then pop off stack
        opd1 = ilStack.peek();
        ilStack.pop();
        opd2 = ilStack.peek();
        ilStack.pop();
        ...
        switch (node->token) {
            case T_ADD:
                cout << "add " << opd2 << "+" << opd1 << "\n";
                ilStack.push( opd2 + "+" + opd1); //push result back onto stack
                break;
            case T_SUB:
                cout << "sub " << opd2 << "-" << opd1 << "\n";
...
void determineActualType(Node* node) {
    ...
    //determine actual type based on children types
...
Node* assign() {
    ...
    //Other than the assign list bug the code runs fine
...
int main () {
    ...
    getChar();
    lex();
    Node* root = program(); //gives us parse tree 

    if (root){
        determineExpectedType(root); //not used in final version but kept it for reference
        determineActualType(root); //determine actual types for type checking
        cout << "\n - IL CODE -\n";
        generateIL(root); //generate IL code from parse tree and actual types
    }
